    ...
    async def get_perception_infos(self, raw_screenshot_file_info: ScreenFileInfo, ui_hierarchy_xml, non_visual_mode=False, target_app=None, use_clickable_node_summaries=True):
        ...
        if non_visual_mode:
            ...
            if self.image_description_generator is not None:
                ...
                error_info = LogTemplate['missing_config'](WorkerType.Tool, "SSIP", "'non_visual_mode=True'", "visual_prompt_model_config", "critical")
                logger.bind(log_tag="fairy_sys").critical(error_info)
            ...
            if use_clickable_node_summaries:
                if self.text_summarizer is not None:
                    ...
                    error_info = LogTemplate['missing_config'](WorkerType.Tool, "SSIP", "'non_visual_mode=True' and 'use_clickable_node_summaries=True'", "text_summarization_model_config", "error")
                    logger.bind(log_tag="fairy_sys").error(error_info)
                    page_desc = await at.get_page_description()
            else:
                warning_info = LogTemplate['function_not_active'](WorkerType.Tool, "SSIP", "Clickable Node Summaries","not setting 'use_clickable_node_summaries'", "'use_clickable_node_summaries=True'")
                logger.bind(log_tag="fairy_sys").warning(warning_info)
                page_desc = await at.get_page_description()

        else:
            logger.bind(log_tag="fairy_sys").info(LogTemplate['function_not_active'](WorkerType.Tool, "SSIP", "Screen Textualized Description","not using 'non_visual_mode' policy", "'non_visual_mode=True'"))
            page_desc = None
